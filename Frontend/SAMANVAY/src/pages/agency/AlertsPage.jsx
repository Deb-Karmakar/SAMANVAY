// src/pages/agency/AlertsPage.jsx
...
export default function AgencyAlertsPage() {
    ...
    const getAlertsForTab = () => {
        ...
        switch (activeTab) {
            ...
            default: return alertsData.allAlerts || [];
        }
    };
        ...
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-muted-foreground">Notifications and tasks requiring your attention.</p>
                </div>
            ...
            <Card>
                <CardHeader><CardTitle>Active Alerts</CardTitle><CardDescription>Click on any alert to view the related project.</CardDescription></CardHeader>
                <CardContent>
                    <Tabs value={activeTab} onValueChange={setActiveTab}>
                        <TabsList className="grid w-full grid-cols-4">
                            <TabsTrigger value="all">All ({alertsData?.total || 0})</TabsTrigger>
                            <TabsTrigger value="critical">Critical ({alertsData?.critical?.length || 0})</TabsTrigger>
                            <TabsTrigger value="warning">Warning ({alertsData?.warning?.length || 0})</TabsTrigger>
                            <TabsTrigger value="info">Info ({alertsData?.info?.length || 0})</TabsTrigger>
                        </TabsList>
                        <div className="mt-6">
                            {alerts.length === 0 ? (
                                <div className="text-center py-12 text-muted-foreground">No {activeTab !== 'all' ? activeTab : ''} alerts</div>
                            ) : (
