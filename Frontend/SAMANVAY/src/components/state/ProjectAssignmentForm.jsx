...
// This function handles the initial assignment (replaces the whole assignments array)
const assignProject = async (payload) => {
    const { data } = await axiosInstance.put(`/projects/${payload.projectId}/assign`, payload);
    return data;
};

// This new function adds assignments to an existing project
...
export default function ProjectAssignmentForm({ project, open, onOpenChange }) {
  ...
  // Determine if we are creating a new assignment or adding to an existing one
  ...
  const { data: allAgencies, isLoading: isLoadingAgencies } = useQuery({ queryKey: ['myStateAgencies'], queryFn: fetchStateAgencies, enabled: open });
  
  // Choose the correct backend function based on the mode (creating vs. adding)
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => {
      ...
      onOpenChange(false);
      alert(`Project ${isEditing ? 'updated' : 'assigned'} successfully!`);
    },
    onError: (error) => alert(`Failed: ${error.response?.data?.message || error.message}`),
  });
  
  // Filter the dropdown to only show agencies that haven't been assigned yet
  ...
  const totalPreviouslyAllocated = useMemo(() => project.assignments?.reduce((sum, a) => sum + a.allocatedFunds, 0) || 0, [project]);
  const totalNewlyAllocated = useMemo(() => Object.values(agencyAllocations).reduce((sum, amount) => sum + (amount || 0), 0), [agencyAllocations]);
  ...
  const handleAssign = () => {
    if (selectedAgencies.length === 0) { alert("Please select at least one new agency."); return; }
    if (remainingBudget < 0) { alert("Total allocated funds cannot exceed the project budget."); return; }
    ...
    if (newAssignments.some(a => a.checklist.length === 0)) { alert("Please add at least one milestone for each newly selected agency."); return; }
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-4xl max-h-[90vh] flex flex-col">
        <DialogHeader>
          <DialogTitle>{isEditing ? 'Add Agencies to Project' : 'Assign Project'}: {project.name}</DialogTitle>
          <DialogDescription>{isEditing ? 'Select new agencies to add to this project.' : 'Select agencies and define their funds and milestones.'}</DialogDescription>
        </DialogHeader>
        ...
        {isEditing && project.assignments.length > 0 && (
            <div className="space-y-2 border-b pb-4">
                ...
                <div className="flex flex-wrap gap-2">
                    {project.assignments.map(a => <Badge key={a.agency._id} variant="outline">{a.agency.name} (₹{a.allocatedFunds}L)</Badge>)}
                </div>
            </div>
        )}
        ...
        <div className="py-4">
            ...
            <Popover open={openPopover} onOpenChange={setOpenPopover}>
                <PopoverTrigger asChild><Button variant="outline" role="combobox" aria-expanded={openPopover} className="w-full justify-between h-auto min-h-10 flex-wrap">{selectedAgencies.length > 0 ? selectedAgencies.map(agency => <Badge key={agency._id} variant="secondary" className="m-0.5">{agency.name}</Badge>) : "Select new agencies to add..."}</Button></PopoverTrigger>
                <PopoverContent className="w-[--radix-popover-trigger-width] p-0"><Command><CommandInput placeholder="Search available agencies..." /><CommandList><CommandEmpty>No available agencies found.</CommandEmpty>
                    <CommandGroup>
                      {availableAgencies.map(agency => (<CommandItem key={agency._id} onSelect={() => setSelectedAgencies(prev => prev.find(a => a._id === agency._id) ? prev.filter(a => a._id !== agency._id) : [...prev, agency])}>
        ...
        <div className="mt-4 flex flex-col gap-2 border-t pt-4">
            <p className="text-sm text-center">Remaining Project Budget: <span className={`font-bold ${remainingBudget < 0 ? 'text-destructive' : ''}`}>₹{remainingBudget.toLocaleString()}</span> Lakhs</p>
            <Button onClick={handleAssign} disabled={mutation.isLoading}>{mutation.isLoading ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : "Confirm & Save Assignments"}</Button>
        </div>
      </DialogContent>
    </Dialog>
  );
